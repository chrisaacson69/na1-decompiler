...
- **HUMAN playability ≠ the AI-vs-AI map.** Against the *player*, the AI uses the harsher grudge gate (share > **47%** ≈ only 0.89× you), so it attacks at near parity. Counting neighbours that clear 47% = "human threats": **8/17 starts are UNPLAYABLE** (3+ threats — incl. Iga, Mino, Musashi, Kaga, Mikawa, Suruga, Yamato, Shinano), most others "hard", and **only Noto and Settsu are PLAYABLE** (0 threats). **Inversion: Noto — the economic runt (header 0) — is the *safest human start*** (1 weak neighbour); the strong central fiefs are death traps. Iga/Mino are unplayable even moving first (Iga needs +10 men it can't afford on 36 gold at start hire rates).
