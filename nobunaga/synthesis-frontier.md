...
- **COULD-vs-WILL caveat (the map's real limit):** the viability "human threats" count is **who *can* grudge-attack — an upper bound, not who *will*.** The grudge override (`$952E`) is conditional: each AI fief acts **once/turn** and usually hits **its own weakest neighbour** (often another AI), only diverting to the human if that didn't fire — so 4 potential threats ≈ 0–1 realized. Plus turn order, plus a gold-rich start (Shinano gold 73) can **buy rice turn-1** to lift its stage-2 bar (rice 40→≥72 men → attackers abort). So `UNPLAYABLE` = "high *potential* exposure," not "certain death." Map updated with the caveat + the Honganji economic flag (⚑).
