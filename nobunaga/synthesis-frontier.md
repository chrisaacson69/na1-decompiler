...
- **Revolt overlay (the RNG-death tail):** a PLAYABLE start with **low morale** can still die turn-1 — a revolt halves men and may open a threat absent at full strength. **Noto** (PLAYABLE, but morale **23**) is the case: revolt → 24 men → Kaga (41) clears 47%. Flagged `REVOLT→dies`; high-morale playables (Settsu 70, Echigo 74) are not. (`MORALE_REVOLT≈50` is a placeholder — the exact revolt probability is the next walk.)
- **Open / refinements:** the **event-trigger walk** (exact revolt/illness probabilities vs morale/loyalty) to replace the `<50` placeholder; cross-check men vs `rom-defaults.txt` for t=0; relations/DRIVE matter on *later* turns (grudges accrue) though not turn-1; per-start strategy prose (view #4, "the detailed human-starts walk").
