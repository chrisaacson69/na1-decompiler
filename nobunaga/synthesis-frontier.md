...
- **Home(0) is strictly better than Balanced(3)** — Home = cascade + 10% daimyo-stat-boost + 25% arms-bonus + the Spring boon (ledger #24); Balanced = cascade only, and (state≠0) it takes the Spring decay. The Grant menu offers only 1–5, so **the player can't grant Home** — but conquered fiefs **default** to it. **Correction to ledger #22:** leaving a conquered fief at Home auto-manages it (full cascade) WITH the subsidies and no decay — *better than granting it for general growth*; Grant is for **focusing** a fief, not automating it. A player's Home fiefs even **auto-wage war** on his behalf (the cascade's military branch runs from them). Census-consistent: a victory save was {Home 33, Balanced 10, Direct 7} — players leave most of the empire at Home.
