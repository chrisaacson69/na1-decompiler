...
- **Other events grounded:** **Flood** (Summer, 50%): `output ×= (0.9·dams)%` — **dams are flood insurance** (no dams → output wiped); + wealth loss. **Ravage** (25%): men & output ×50–99%, **and at a capital the daimyo loses `rng(9)+1` health** (the only random event that hits a life pool). **Illness** (every season, `rng(400)<100−health`) → "has taken ill". **Natural death** (every season, age≥70 + health roll). **Aging** (Spring, age+1/health−1/yr). **AI-initiated** (`random_event_type_dispatch`, 50%): AI marriage / diplomacy / extra ravage — the AI using the player's diplomacy verbs.
- **⚠BIG (verify): the Spring rubber-band.** In the aging pass, **state-0 (AI) fiefs get a wealth/loyalty BOON** (`event_boost_province_wealth_loyalty`: +rng(10)+Cha/10 wealth, +rng(40)+Cha/10 loyalty, 50%), while **state-≠0 (player Direct + Granted) fiefs DECAY 3 stats by `rng(const_two)`** at difficulty>1. So the AI is *subsidized* and the player *taxed*, scaled by difficulty — and **Granting a fief off Home(0) exposes it to the decay** (a hidden cost of delegation). Offsets read from murky decompiler pointer math → bytecode-verify before promoting.
