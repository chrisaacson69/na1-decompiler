...
<style>
  @font-face {{ font-family:'NA'; src:url('assets/fonts/nobunaga.ttf') format('truetype'); }}
  ...
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  /* ---- flashy hero: the player draw ---- */
  .hero {{ background:#0a0a12; text-align:center; padding:40px 20px 46px; border-bottom:3px solid #45e082; }}
  .hero-art {{ display:block; margin:0 auto; width:100%; max-width:512px; image-rendering:pixelated; }}
  .hero-menu {{ display:flex; justify-content:center; gap:54px; margin-top:26px; font-family:'NA',monospace; }}
  .hm-col {{ text-align:left; }}
  .hm-h {{ display:block; color:#fffeff; font-size:23px; letter-spacing:2px; margin-bottom:8px; }}
  .hero-menu a {{ display:block; color:#45e082; font-size:20px; margin:5px 0; text-decoration:none; }}
  .hero-menu a:hover {{ text-decoration:underline; }}
  .hero-tag {{ max-width:620px; margin:24px auto 0; color:#9a9ab0; font-style:italic; }}

  /* ---- light content: the deeper layers ---- */
  .content {{ max-width:860px; margin:0 auto; padding:50px 36px; }}
  h1 {{ font-size:2.0em; color:var(--accent); line-height:1.15; margin-bottom:6px; }}
  .subtitle {{ font-size:1.08em; color:var(--muted); font-style:italic; margin-bottom:20px;
              padding-bottom:16px; border-bottom:2px solid var(--accent); }}
  .lead {{ font-size:1.08em; margin-bottom:8px; }}
  h2 {{ font-size:1.3em; color:var(--accent); margin-top:36px; margin-bottom:13px;
  ...
  .nav-cards {{ display:grid; grid-template-columns:repeat(auto-fit,minmax(200px,1fr)); gap:14px; margin:30px 0 8px; }}
  ...
  @media (max-width:640px) {{ .hero-menu {{ gap:30px; }} .content {{ padding:34px 18px; }} }}
...
<header class="hero">
  <img class="hero-art" src="assets/screens/title-home.png" alt="Nobunaga's Ambition (NES)">
  <nav class="hero-menu">
    <div class="hm-col"><span class="hm-h">Fiefs</span>
      <a href="atlas/fief-17.html">17-fief map</a>
      <a href="atlas/fief-50.html">50-fief map</a></div>
    <div class="hm-col"><span class="hm-h">Daimyo</span>
      <a href="atlas/daimyo-17.html">17-fief lords</a>
      <a href="atlas/daimyo-50.html">50-fief lords</a></div>
  </nav>
  <p class="hero-tag">Click the map &mdash; every province and lord, with stats, portraits, banners and
  battle maps, rendered from the ROM in the game's own font.</p>
</header>

<main class="content">

<h1>Under the hood</h1>
<p class="subtitle">The 1989 Koei strategy classic, lifted from stack-VM bytecode to readable C &mdash; then mined for how the engine actually works.</p>

<p class="lead">The maps and stats above are the surface. Below is the research that produced them:
the full decompilation, chapter-by-chapter analysis, and bytecode-verified formulas. Everything
here was recovered from the ROM &mdash; see <a href="https://github.com/chrisaacson69/na1-decompiler">the code</a>.</p>
