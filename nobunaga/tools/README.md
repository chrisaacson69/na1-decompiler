...
| `vm-disasm.py` | ✅ | **Bulk VM bytecode disassembler — RECOVERED 2026-05-30** (had drifted out of `tools/`, ch6). `py -3 tools/vm-disasm.py <bank 0-15> [-o out.asm] [--stdout] [--check]`. Walks every `JSR $E823` bytecode sub in a bank → readable VM-asm. Decode lengths come from the **execution-validated** `nobunaga_vm.OPCODE_INFO` (NOT the deleted old spec, which had wrong operand lengths — see ROADMAP drift note); `$D5`/`$D9` variable-length inline jump tables decoded from their handlers. `--check` runs the **exact-tiling** correctness test (135 stubs must tile the bank). ⚠️ Does **not** clobber the 4 hand-annotated `disasm/*_vm.asm`; its job is fresh banks. |
