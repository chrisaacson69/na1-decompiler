...
| `cluster-anon-subs.py` | ✅ | **Anon-sub clusterer for the label-walk (2026-06-01).** `py -3 tools/cluster-anon-subs.py <bank> [--size N] [--json]`. Reads `decompiled/bank_NN.c`, buckets every still-anonymous `sub_XXXX` by sorted address into ~N-sized clusters (one cluster = one PROPOSER agent's batch). The GENERALIZATION of the hand-coded `CLUSTERS` in the bank01-label-walk Workflow — feeds the label-walk skill via `--json` → `args`. Deterministic. (bank_00 = 97 anon → 14 clusters; bank_01 = 0, confirming it's done.) |
