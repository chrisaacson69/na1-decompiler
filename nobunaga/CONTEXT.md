...
| `source/4-c/all_banks.c` | the flat PRG-keyed merged view (all 4 code banks, one file) | DERIVED from the per-bank `.c` via `tools/decompile-merged.py`; for grepping a label / following a call across banks. Per-bank `bank_NN.c` stay the canonical regen-guarded source. |
...
| `source/2-asm-vm/bank_NN_vm.asm` | per-bank VM bytecode disasm | `*_labeled.asm` = native+labels; `*_vm.asm` = VM expansion. |
...
- **Var names (var-walk, M4)** — frame slots decompile positionally (`arg1..arg4`, `local0..local11`); semantic names live in `mesen-labels.toml` `[vars.bankN."0xADDR"]` sections (ADDR = the sub's stub), one `slot = { name, comment }` line each. `vm_decompile.py` rewrites them as the LAST emit pass, so `arg1->output` becomes `fief->output`. **Rule:** only name a slot with ONE coherent role — reused scratch (disjoint live ranges, different meanings) STAYS positional, which is the honest "this is scratch" signal. Recovered by usage-role (body) + caller-propagation (the M3 reverse-push fix makes each caller's pushed expr visible). Pilot: `effect_grow` `$87F0`. (To add: edit the toml + `decompile-all.py` regen; diff must be pure renames.)
- **ext-op / `math32_3arg` / `math32_2arg` / `pct_op`** — the VM's 32-bit arithmetic helpers (bank-15 bytecode subs, built on the `B7` extended opcodes). **DERIVED 2026-05-29** (`tools/probe-math32.py`): `pct_op(a,b)=⌊ab/100⌋`, `math32_3arg(a,b,c)=⌊ab/c⌋`, `math32_2arg(a,b)=⌊100a/(a+b)⌋`. No longer a blocker. (The B7 ext-op sub-op table is now FULLY DECODED too, 2026-05-31: `ext_op_table` $F263, 47 active handlers — umul32/udiv32/sdiv32/umod32/add32/cmp32_*/bitfield/etc. — all named in `mesen-labels.toml [prg.bank15]`; `vm_decompile.py EXT_OP_TABLE` resolves the index→name so `source/4-c/*.c` reads the real op, not `// TODO: ext_op $NN`. The 3 composite helpers remain the *formula* interface; the leaves are now readable for decoding the rest.)
...
- **Validate the VM model (Pass 2):** Rung 1 `tools/vm-roundtrip.py` (bytes re-encode byte-identical to ROM) + Rung 2 `tools/vm-stack-audit.py` (every opcode's stack/reg effect vs the live ROM handler, gated). `tools/vm_stack_effect.py` is the per-opcode `STACK_EFFECT` authority both feed off; `vm-opcodes-v2.toml` `verified=`/`pops`/`pushes` are written from a clean audit (`--write-toml`). **M3 drift-guard:** `py -3 tools/decompile-all.py --self-check` asserts the *decompiler's* observed data-stack Δ per opcode (real handlers, default-off `SELF_CHECK` trace) matches `STACK_EFFECT` — the standing guard against the next $30-$3F/$B2-class bug (catches push-modeled-as-store etc.).
- **Search/follow code across ALL 4 banks at once (flat PRG view):** read `source/4-c/all_banks.c` (regen automatically with the per-bank files via `tools/decompile-all.py`, or standalone via `tools/decompile-merged.py`) — every sub keyed by PRG offset (`L_pPPPPP`, `// PRG $P`), each call annotated `// -> bankT $CPU`, the `call_bank_wrap(N)`/`call_bank10_entry` trampolines resolved to the real entry. Derived/searchable view; `source/4-c/bank_NN.c` remain canonical.
...
### ⭐ STANDING GOAL: label as you go (Chris, 2026-06-02)
**Every session that identifies a code address, ROM table, RAM/SRAM variable, or data blob MUST write that understanding back into `mesen-labels.toml` before it ends** — this is not optional cleanup, it is the deliverable. An address you decoded but left named `foo_data_8d5c` is a label the *next* session has to re-derive (the exact drift this project exists to kill). The mechanics are the `/data-walk` (data) and `/label-walk` (code) skills, or for a handful of finds just edit the toml directly + regen (`decompile-all.py`) + confirm the diff is **pure renames** (the regen-guard). Naming rule: *vague-but-correct beats specific-but-wrong* — a domain+role name with the uncertainty in the comment. Treat "did I name what I found?" as a session-exit checklist item alongside updating this file + the ROADMAP. (Worked example: the 2026-06-02 strategic-map session named `strategic_map_section_tilemaps`/`_chr_tiles`/`_attributes`/`_bg_palette` + `province_to_map_section_17`/`_50`, all regen-guard-clean.)
